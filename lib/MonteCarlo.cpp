        ...
        for(int j = 0; j < MCP.cellsB; j++){
            for(int z = 0; z < MCP.cellsC; z++){
                for(int k = 0; k < NUMSUBLATTICES; k++){
                    ...
                    PX += (X * a1_x.getX() + Y * a1_x.getY() + Z * a1_x.getZ());
                    PY += (X * a1_y.getX() + Y * a1_y.getY() + Z * a1_y.getZ());
...
void MonteCarlo::updateCorrelationFunction(){
    ...
    for(int count = 0; count < 2*MCP.cellsA; count++){
        ...
        PX[count] += (X * a1_x.getX() + Y * a1_x.getY() + Z * a1_x.getZ());
        PY[count] += (X * a1_y.getX() + Y * a1_y.getY() + Z * a1_y.getZ());
...
double MonteCarlo::getLocalEnergy(const ClassicalSpin3D& a, const bool fromSpinFlip) const{
    ...
    double interactionCubic = MCP.cubicD * (-1.0) * a.getZ() * a.getZ();
    ...
    //B field
    if (MCP.isBField) {
        double interactionB = (-1) * (MCP.bField_xNorm * a.getX() +
                                      MCP.bField_yNorm * a.getY() +
                                      MCP.bField_zNorm * a.getZ());
