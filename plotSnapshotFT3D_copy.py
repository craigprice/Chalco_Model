...
def isInside1stBZ2DHoneycomb(px_kx, py_ky):
    #cout<<"py_ky "<<py_ky<<
    ...
    for i in range (0,3):
        #print 'limit on min py_ky', ((slopePerp_NN[i]*px_kx + bcoor_NN[i])-1e-7)
        if py_ky > ((slopePerp_NN[i]*px_kx + bcoor_NN[i])-1e-7):
            pass #inside BZ
        else:
            isAllTrue = False
    for i in range (3,6):
        #print 'limit on max py_ky', ((slopePerp_NN[i]*px_kx + bcoor_NN[i])+1e-7)
        if py_ky < ((slopePerp_NN[i]*px_kx + bcoor_NN[i])+1e-7):
            pass #inside BZ
        else:
            isAllTrue = False
...
def isInside2ndBZ2DHoneycomb(px_kx, py_ky):
    #print "px_kx ", px_kx, "py_ky ", py_ky
...
def pointIn1stBZ( numPointsInRecSpaceX,  linSizeX,
    ...
    for i_k in range (-1,2,1):
        for j_k in range (-1,2,1):
            ...
            #print 'i_k',i_k, 'j_k', j_k, "numPointsInRecSpaceX ", numPointsInRecSpaceX, " numPointsInRecSpaceY ", numPointsInRecSpaceY, " numPointsInRecSpaceSublattice ", numPointsInRecSpaceSublattice," p.x ", p.x[i_k+1][j_k+1]/PI, " p.y ", p.y[i_k+1][j_k+1]/PI
            if isInside1stBZ2DHoneycomb(p.x[i_k+1][j_k+1], p.y[i_k+1][j_k+1]):
...
def pointIn2ndBZ( numPointsInRecSpaceX,  linSizeX,
    numPointsInRecSpaceY,  linSizeY,
    numPointsInRecSpaceSublattice,  numSubLattices,
    magSk):
    p = allPointsInBZ()
    ...
    for i_k in range (-1,2,1):
        for j_k in range (-1,2,1):
            ...
            if isInside2ndBZ2DHoneycomb(p.x[i_k+1][j_k+1], p.y[i_k+1][j_k+1]):
                p.isExists[i_k+1][j_k+1]=True
                p.magSk[i_k+1][j_k+1]=magSk
    for i in range (0,3,1):
        for j in range (0,3):
...
pathToSimFiles ='/Volumes/Toshi/dataFromSimulations/honV9K12/hep2p/dataFiles_2015_01_11_2BZ'
...
for f in simFiles:
    ...
    for line in sim_file:
        ...
        if BZ == '2':
            p = pointIn2ndBZ(xPos,cellsA,yPos,cellsB,1,1, 0)
        if BZ == '1':
            if sPos > 0: continue
            #p = pointIn1stBZ(xPos,cellsA,yPos,cellsB,sPos,2,0)
            p = pointIn1stBZ(xPos,cellsA,yPos,cellsB,1,1,0)
        r_x = 0
        r_y = 0
        for i in range (0,3,1):
            for j in range (0,3):
    ...
    #plt.title(graphTitle)
    font_size = 24
    plt.title(graphTitle,y=1.03,fontsize=font_size)
    plt.xlabel(r'$k_x$',fontsize=font_size)
    plt.ylabel(r'$k_y$',fontsize=font_size)
    ...
    #plt.savefig('figuresFT/eps/3D_'+ graphTitle.replace('/','_').strip() + '.eps',dpi=150)
    #plt.savefig('figuresFT/3D_'+ graphTitle.replace('/','_').strip() + '.png',dpi=100)
    plt.savefig('figures2BZ/3D/'+ graphTitle.strip().replace('\\','') + '.pdf',dpi=100)
    #plt.show()
    plt.close('all')
    #outfile.close()
    if f_count > 1:
